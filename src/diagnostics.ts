...
export interface WebContextConfig {
  appName: string
  appVersion: string
  env: string // "production" | "alpha" | "dev"
}
...
export function osMajorFromUA(ua?: string): string {
  if (!ua) return ''
...
export function osFromUA(ua?: string): string {
  ...
  if (/iPhone|iPad|iPod/.test(ua) || /(?:iPhone OS|CPU OS)\s+\d/.test(ua)) return 'ios'
...
export function deviceFromUA(ua?: string): string {
  ...
  if (/Mobi|Windows Phone/.test(ua)) return 'phone'
  return 'desktop'
}

export function gatherWebContext(cfg: WebContextConfig): BeaconContext {
