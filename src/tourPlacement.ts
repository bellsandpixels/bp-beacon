...
export function placeTourCard(target: Rect | null, card: Size, viewport: Size): CardPlacement {
  ...
  if (roomBelow >= card.height) return { top: below, left, side: 'below' }
  if (roomAbove >= 0) return { top: above, left, side: 'above' }
