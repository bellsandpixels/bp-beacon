...
export function surfacesOf(catalogue: ValidationCatalogue): WalkSurface[] {
  const out: WalkSurface[] = []
  for (const area of catalogue.areas) for (const s of area.surfaces) out.push(s)
  return out
}

export function checkByNumber(catalogue: ValidationCatalogue, n: number): WalkCheck | undefined {
  for (const s of surfacesOf(catalogue)) for (const c of s.checks) if (c.n === n) return c
  return undefined
}

// A defect counts only when its note is non-empty after trim (an in-progress flag is not a defect).
function recordedDefects(state: WalkState): number[] {
  const out: number[] = []
  for (const key of Object.keys(state.defects)) {
    const n = Number(key)
    const note = state.defects[n]
    if (note != null && String(note).trim().length > 0) out.push(n)
  }
  return out
}

export function deriveVerdict(catalogue: ValidationCatalogue, state: WalkState): Verdict {
  const surfaces = surfacesOf(catalogue)
  const total = surfaces.length
  let walked = 0
  for (const s of surfaces) if (state.walked[s.key]) walked++

  const defectNs = recordedDefects(state)
  let blockers = 0
  for (const n of defectNs) {
    const chk = checkByNumber(catalogue, n)
    if (chk && chk.severity === 'blocker') blockers++
  }
  ...
  if (blockers > 0) label = 'BLOCKED'
  else if (defects > 0) label = 'FAIL'
  else if (walked === 0) label = 'NOT REVIEWED'
  else if (walked < total) label = 'INCOMPLETE'
  else label = 'PASS'
